...
audio_backchannel_module_mapper & audio_backchannel_module_mapper::instance( void )
{
	static audio_backchannel_module_mapper _instance;
	return _instance;
}

audio_backchannel_module_mapper::audio_backchannel_module_mapper( void )
	: _is_mapped(false)
{
...
unsigned short audio_backchannel_module_mapper::get_module_vendor_information( int ***vendor_ids, char ***vendor_names, int *length )
{
	load();
	std::map<int,std::string> vendor_ids_names;
	std::map<unsigned int,vendor>::iterator vendor_iter;
	for( vendor_iter=_audio_backchannel_vendors.begin(); vendor_iter!=_audio_backchannel_vendors.end(); vendor_iter++ )
	{
...
unsigned short audio_backchannel_module_mapper::get_module_vendor_device_informations( int vendor_id, int ***vendor_device_ids, char ***vendor_device_names, int *length )
{
	load();
	std::map<int,std::string> vendor_device_ids_names;
	std::map<unsigned int,vendor>::iterator vendor_iter;
	vendor_iter = _audio_backchannel_vendors.find( vendor_id );
	if( vendor_iter!=_audio_backchannel_vendors.end() )
	{
		vendor single_vendor = (*vendor_iter).second;
...
unsigned short audio_backchannel_module_mapper::get_module_vendor_device_protocol_informations( int vendor_id, int vendor_device_id, int ***vendor_device_protocol_ids, char ***vendor_device_protocol_names, int *length )
{
	load();
...
unsigned short audio_backchannel_module_mapper::get_module_device_version_informations( int vendor_id, int vendor_device_id, int vendor_device_protocol_id, int ***vendor_device_version_ids, char ***vendor_device_version_names, int *length )
{
...
void audio_backchannel_module_mapper::get_module_name( unsigned int vendor_id, unsigned int vendor_device_id, unsigned int protocol_id, unsigned int firmware_version_id, char *module_name )
{
	load();
	std::map<unsigned int,vendor>::iterator vendors_iter;
	vendors_iter = _audio_backchannel_vendors.find( vendor_id );
	if( vendors_iter!=_audio_backchannel_vendors.end() )
	{
...
void audio_backchannel_module_mapper::load( void )
{
	...
	if( !_is_mapped )
	{
		HINSTANCE self;
#if defined(_DEBUG)
		self = ::GetModuleHandleA( "audio_backchannel_controllerd.dll" );
#else
		self = ::GetModuleHandleA( "audio_backchannel_controller.dll" );
#endif
...

		{
			...
			HINSTANCE instance;
			base_audio_backchannel_controller	*controller;
			while( result )
			{
				if( (wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)!=FILE_ATTRIBUTE_DIRECTORY )
				{
					...
					if( strlen(szModulePath)>0 )
					{
						...
						if( instance )
						{
							pfnCreateAudioBackChannelController pfnCreate = (pfnCreateAudioBackChannelController)::GetProcAddress( instance, "create" );
							if( pfnCreate )
							{
								...
								{
									std::map<unsigned int,vendor>::iterator vendors_iter;
									vendors_iter = _audio_backchannel_vendors.find( controller->get_vendor_id() );
									if( vendors_iter!=_audio_backchannel_vendors.end() )
									{
										vendor single_vendor = (*vendors_iter).second;
										std::map<unsigned int,device>::iterator devices_iter;
										...
										if( devices_iter!=single_vendor.device_list.end() )
										{
											...
											if( protocols_iter!=single_device.protocol_list.end() )
											{
												...
												_audio_backchannel_vendors.erase( controller->get_vendor_id() );
												_audio_backchannel_vendors.insert( std::make_pair(controller->get_vendor_id(), single_vendor) );
											}
											else
											{
												firmware_version single_firmware_version;
									...
									pfnDestroyAudioBackChannelController pfnDestroy = (pfnDestroyAudioBackChannelController)::GetProcAddress( instance, "destroy" );
									if( controller )
									{
