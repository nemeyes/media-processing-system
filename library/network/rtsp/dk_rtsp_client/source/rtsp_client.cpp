...
debuggerking::rtsp_core * debuggerking::rtsp_core::createNew(rtsp_client * front, UsageEnvironment & env, const char * url, const char * username, const char * password, int transport_option, int recv_option, int recv_timeout, float scale, unsigned int http_port_number, bool * kill_flag)
{
	return new rtsp_core(front, env, url, username, password, transport_option, recv_option, recv_timeout, scale, http_port_number, kill_flag);
}

debuggerking::rtsp_core::rtsp_core(rtsp_client * front, UsageEnvironment & env, const char * url, const char * username, const char * password, int transport_option, int recv_option, int recv_timeout, float scale, unsigned int http_port_number, bool * kill_flag)
	...
	, _pps(0)
{
	...
	_kill_trigger = envir().taskScheduler().createEventTrigger((TaskFunc*)&(rtsp_core::kill_trigger));
	//_task_sched = &_env->taskScheduler();
	if (username && password && strlen(username)>0 && strlen(password)>0)
        _auth = new Authenticator(username, password);
}

debuggerking::rtsp_core::~rtsp_core(void)
{
	if (_auth)
	{
		delete _auth;
...
void debuggerking::rtsp_core::get_options(RTSPClient::responseHandler * after_func)
{
    sendOptionsCommand(after_func, _auth);
}

void debuggerking::rtsp_core::get_description(RTSPClient::responseHandler * after_func)
{
    sendDescribeCommand(after_func, _auth);
}

void debuggerking::rtsp_core::setup_media_subsession(MediaSubsession * media_subsession, bool rtp_over_tcp, bool force_multicast_unspecified, RTSPClient::responseHandler * after_func)
{
    sendSetupCommand(*media_subsession, after_func, False, rtp_over_tcp?True:False, force_multicast_unspecified?True:False, _auth);
}

void debuggerking::rtsp_core::start_playing_session(MediaSession * media_session, double start, double end, float scale, RTSPClient::responseHandler * after_func)
{
    sendPlayCommand(*media_session, after_func, start, end, scale, _auth);
}

void debuggerking::rtsp_core::start_playing_session(MediaSession * media_session, const char * abs_start_time, const char * abs_end_time, float scale, RTSPClient::responseHandler * after_func)
{
...
void debuggerking::rtsp_core::set_user_agent_string(const char * user_agent)
{
    setUserAgentString(user_agent);
}

void debuggerking::rtsp_core::continue_after_client_creation(RTSPClient * param)
{
	rtsp_core * self = static_cast<rtsp_core*>(param);
    self->set_user_agent_string("DebuggerKing ICT");
    self->get_options(continue_after_options);
}

void debuggerking::rtsp_core::continue_after_options(RTSPClient * param, int result_code, char * result_string)
{
	rtsp_core * self = static_cast<rtsp_core*>(param);
...
void debuggerking::rtsp_core::continue_after_describe(RTSPClient * param, int result_code, char * result_string)
{
	rtsp_core * self = static_cast<rtsp_core*>(param);
	...
	do
	{
		...
		char medium[10] = { 0 };
		if (self->_recv_option & rtsp_client::recv_option_t::audio)
			strncpy(medium, "audio", sizeof(medium));
		if (self->_recv_option & rtsp_client::recv_option_t::video)
...
void debuggerking::rtsp_core::continue_after_setup(RTSPClient * param, int result_code, char * result_string)
{
	rtsp_core * self = static_cast<rtsp_core*>(param);
...
void debuggerking::rtsp_core::continue_after_play(RTSPClient * param, int result_code, char * result_string)
{
	rtsp_core * self = static_cast<rtsp_core*>(param);
...
void debuggerking::rtsp_core::close(void)
{
	//envir().taskScheduler();
	envir().taskScheduler().triggerEvent(_kill_trigger, this);
}

void debuggerking::rtsp_core::subsession_after_playing(void * param)
{
	MediaSubsession * media_subsession = static_cast<MediaSubsession*>(param);
	rtsp_core * self = static_cast<rtsp_core*>(media_subsession->miscPtr);
...
void debuggerking::rtsp_core::subsession_bye_handler(void * param)
{
	MediaSubsession * media_subsession = static_cast<MediaSubsession*>(param);
	rtsp_core * self = static_cast<rtsp_core*>(media_subsession->miscPtr);
...
void debuggerking::rtsp_core::session_after_playing(void * param)
{
	rtsp_core * self = static_cast<rtsp_core*>(param);
#if 0
	self->shutdown();
...
void debuggerking::rtsp_core::setup_streams(void)
{
	...
	while ((media_subsession = _iter->next()) != 0)
    {
        if( media_subsession->clientPortNum()==0 )
            continue;
        if(_transport_option==rtsp_client::rtp_over_tcp)
			setup_media_subsession(media_subsession, true, false, rtsp_core::continue_after_setup);
        else
			setup_media_subsession(media_subsession, false, false, rtsp_core::continue_after_setup);
...
void debuggerking::rtsp_core::shutdown(void)
{
    if( _shutting_down )
        return;
...
void debuggerking::rtsp_core::session_timer_handler(void * param)
{
	rtsp_core * self = static_cast<rtsp_core*>(param);
...
void debuggerking::rtsp_core::check_packet_arrival(void * param)
{
	rtsp_core * self = static_cast<rtsp_core*>(param);
    int delay_usec = 100000; //100 ms
	self->_arrival_check_timer_task = self->envir().taskScheduler().scheduleDelayedTask(delay_usec, (TaskFunc*)&rtsp_core::check_packet_arrival, self);
}

void debuggerking::rtsp_core::check_inter_packet_gaps(void * param)
{
	rtsp_core * self = static_cast<rtsp_core*>(param);
    ...
    else
    {
		...
		self->_inter_packet_gap_check_timer_task = self->envir().taskScheduler().scheduleDelayedTask(self->_inter_packet_gap_max_time * 1000000, (TaskFunc*)&rtsp_core::check_inter_packet_gaps, self);
    }
}

void debuggerking::rtsp_core::check_session_timeout_broken_server(void * param)
{
	rtsp_core * self = static_cast<rtsp_core*>(param);
	...
	self->_session_timeout_broken_server_task = self->envir().taskScheduler().scheduleDelayedTask(delay_sec_until_next_keepalive * 1000000, (TaskFunc*)&rtsp_core::check_session_timeout_broken_server, self);
}

void debuggerking::rtsp_core::kill_trigger(void * param)
{
	rtsp_core * self = static_cast<rtsp_core*>(param);
	self->_shutting_down = false;
	self->shutdown();
