...
{
public:
	static buffer_sink * createNew(dk_rtsp_client * front, dk_rtsp_client::MEDIA_TYPE_T mt, int32_t smt, UsageEnvironment & env, unsigned buffer_size);

    virtual void add_data(unsigned char * data, unsigned size, struct timeval presentation_time);

protected:
	buffer_sink(dk_rtsp_client * front, dk_rtsp_client::MEDIA_TYPE_T mt, int32_t smt, UsageEnvironment & env, unsigned buffer_size);
...
protected:
	...
	dk_rtsp_client::VIDEO_SUBMEDIA_TYPE_T _vsmt;
	dk_rtsp_client::AUDIO_SUBMEDIA_TYPE_T _asmt;
};
