...
{
public:
	static buffer_sink * createNew(debuggerking::rtsp_client * front, int32_t mt, int32_t smt, UsageEnvironment & env, unsigned buffer_size);

    virtual void add_data(unsigned char * data, unsigned size, struct timeval presentation_time);

protected:
	buffer_sink(debuggerking::rtsp_client * front, int32_t mt, int32_t smt, UsageEnvironment & env, unsigned buffer_size);
...
protected:
	...
	debuggerking::rtsp_client * _front;
