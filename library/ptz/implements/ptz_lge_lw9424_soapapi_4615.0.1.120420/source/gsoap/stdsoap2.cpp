...
{ register int err = soap->errnum;
  if (err)
  {
    ...
    *soap->msgbuf = '\0';
    ...
    DWORD i, len;
    *soap->msgbuf = '\0';
    len = FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, err, 0, (LPTSTR)soap->msgbuf, (DWORD)(sizeof(soap->msgbuf)/sizeof(char)), NULL);
    for (i = 0; i <= len; i++)
    { if (((char*)soap->msgbuf)[i] < 0x80)
        soap->msgbuf[i] = (char)((char*)soap->msgbuf)[i];
      else
        soap->msgbuf[i] = '?';
    }
#endif
