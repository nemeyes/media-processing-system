...
{
	return VMS_PTZ_DEVICE_INFO[HIKVISION_DS_2DF5274_A_REST_V_5_1_4_131219][VENDOR];
}

char* hikvision_ptz_controller::get_vendor_device_name( void )
{
	return VMS_PTZ_DEVICE_INFO[HIKVISION_DS_2DF5274_A_REST_V_5_1_4_131219][DEVICE];
}

char* hikvision_ptz_controller::get_vendor_device_protocol_name( void )
{
	return VMS_PTZ_DEVICE_INFO[HIKVISION_DS_2DF5274_A_REST_V_5_1_4_131219][PROTOCOL];
}

char* hikvision_ptz_controller::get_vendor_device_version_name( void )
{
	return VMS_PTZ_DEVICE_INFO[HIKVISION_DS_2DF5274_A_REST_V_5_1_4_131219][VERSION];
}

unsigned short hikvision_ptz_controller::get_vendor_id( void )
{

	return VMS_PTZ_DEVICE_ID[HIKVISION_DS_2DF5274_A_REST_V_5_1_4_131219][VENDOR];
}

unsigned short hikvision_ptz_controller::get_vendor_device_id( void )
{
	return VMS_PTZ_DEVICE_ID[HIKVISION_DS_2DF5274_A_REST_V_5_1_4_131219][DEVICE];
}

unsigned short hikvision_ptz_controller::get_vendor_device_protocol_id( void )
{
	return VMS_PTZ_DEVICE_ID[HIKVISION_DS_2DF5274_A_REST_V_5_1_4_131219][PROTOCOL];
}

unsigned short hikvision_ptz_controller::get_vendor_device_version_id( void )
{
	return VMS_PTZ_DEVICE_ID[HIKVISION_DS_2DF5274_A_REST_V_5_1_4_131219][VERSION];
}
...
unsigned short hikvision_ptz_controller::get_prest_list( void )
{
	...
	http_client client( _hostname, _port_number, "/ISAPI/PTZCtrl/channels/1/presets" );	
...
unsigned short hikvision_ptz_controller::add_preset2( int &alias )
{
	...
	char url_data[50] = {0};
	snprintf( url_data, sizeof(url_data), "/ISAPI/PTZCtrl/channels/1/presets/%d", alias );
...
unsigned short hikvision_ptz_controller::goto_preset2( int alias )
{
	...
	char url_data[50] = {0};
	snprintf( url_data, sizeof(url_data), "/ISAPI/PTZCtrl/channels/1/presets/%d/goto", alias );
	http_client client( _hostname, _port_number, url_data );
...
unsigned short hikvision_ptz_controller::remove_preset_tour2( int tour_name )
{
    ...
    snprintf( url_data, sizeof(url_data), "/ISAPI/PTZCtrl/channels/1/patrols/%d", tour_name );
	http_client client( _hostname, _port_number, url_data );
	if( !client.send_request(_user_id, _user_password, "DELETE") ) value = VMS_PTZ_FAIL;
	else value = VMS_PTZ_SUCCESS;
...
unsigned short hikvision_ptz_controller::operate_preset_tour2( int tour_name, PTZ_TOUR_CMD_TYPE_T cmd )
{
	...
	switch( UINT8(cmd) )
	{
		case PTZ_TOUR_CMD_START  :		
			snprintf( url_data, sizeof(url_data), "/ISAPI/PTZCtrl/channels/1/patrols/%d/start", tour_name );			
			break;

		case PTZ_TOUR_CMD_STOP :
			snprintf( url_data, sizeof(url_data), "/ISAPI/PTZCtrl/channels/1/patrols/%d/stop", tour_name );
			break;
...
unsigned short hikvision_ptz_controller::set_preset_tour( PTZ_TOUR_T *tour )
{
	...
	unsigned short value = VMS_PTZ_FAIL;
	char url_data[50] = {0};
	snprintf( url_data, sizeof(url_data), "/ISAPI/PTZCtrl/channels/1/patrols/%s", tour->tour_name );
	http_client client( _hostname, _port_number, url_data );
	...
	sprintf(tmp_str, sizeof(tmp_str), "<enabled>true</enabled><id>%s</id><patrolName>%s</patrolName>", tour->tour_name, tour->tour_name);	
	...
	for(int i=0; i<tour->size_of_tour_spots;i++)
	{
		...
		xml_str.append(tmp_str);
		snprintf( tmp_str, sizeof(tmp_str), "<seqSpeed>%d</seqSpeed>", int(get_speed_sensitive_value(tsp->speed)));
		xml_str.append(tmp_str);
		xml_str.append("</PatrolSequence>");
	}
	xml_str.append("</PatrolSequenceList>");
...
unsigned short hikvision_ptz_controller::get_preset_tour( PTZ_TOUR_T *tour )
{
	...
	if( response_data )
	{
		...
		for(elem = elem_spot_list->FirstChildElement("PatrolSequence");elem != NULL; elem = elem->NextSiblingElement() )
		{
			...
			elem_speed = elem->FirstChildElement("seqSpeed");
			if( elem_speed )
			{
...
{
	unsigned short value = VMS_PTZ_UNSUPPORTED_COMMAND;
	http_client client( _hostname, _port_number, "/ISAPI/PTZCtrl/channels/1/continuous" );
