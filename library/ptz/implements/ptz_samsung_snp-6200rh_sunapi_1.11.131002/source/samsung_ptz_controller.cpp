...
{
	return VMS_PTZ_DEVICE_INFO[SAMSUNG_SNP_6200RH_SUNAPI_V_1_11_131002][VENDOR];
}

char* samsung_ptz_controller::get_vendor_device_name( void )
{
	return VMS_PTZ_DEVICE_INFO[SAMSUNG_SNP_6200RH_SUNAPI_V_1_11_131002][DEVICE];
}

char* samsung_ptz_controller::get_vendor_device_protocol_name( void )
{
	return VMS_PTZ_DEVICE_INFO[SAMSUNG_SNP_6200RH_SUNAPI_V_1_11_131002][PROTOCOL];
}

char* samsung_ptz_controller::get_vendor_device_version_name( void )
{
	return VMS_PTZ_DEVICE_INFO[SAMSUNG_SNP_6200RH_SUNAPI_V_1_11_131002][VERSION];
}

unsigned short samsung_ptz_controller::get_vendor_id( void )
{

	return VMS_PTZ_DEVICE_ID[SAMSUNG_SNP_6200RH_SUNAPI_V_1_11_131002][VENDOR];
}

unsigned short samsung_ptz_controller::get_vendor_device_id( void )
{
	return VMS_PTZ_DEVICE_ID[SAMSUNG_SNP_6200RH_SUNAPI_V_1_11_131002][DEVICE];
}

unsigned short samsung_ptz_controller::get_vendor_device_protocol_id( void )
{
	return VMS_PTZ_DEVICE_ID[SAMSUNG_SNP_6200RH_SUNAPI_V_1_11_131002][PROTOCOL];
}

unsigned short samsung_ptz_controller::get_vendor_device_version_id( void )
{
	return VMS_PTZ_DEVICE_ID[SAMSUNG_SNP_6200RH_SUNAPI_V_1_11_131002][VERSION];
}
...
{
	if(_preset_map.size()==0) get_prest_list();
	///cgi-bin/ptzgroup.cgi?action=view
	http_client client( _hostname, _port_number, "/cgi-bin/ptzgroup.cgi" );
	client.put_variable( "action", "view" );
	std::string groupId = "Group";
	groupId.append(tour->tour_name);
	...
	for( v_it=vec.begin(); v_it!=vec.end(); v_it++ )
	{
		...
		if (pair.find(groupId) != pair.npos) {
			split2map( pair.substr(7), ":", &tour_map );
		}
	}
	...
	tour->size_of_tour_spots = (tour_map.size()-1)/3;
    tour->tour_spots = new PTZ_TOUR_SPOT_T[tour->size_of_tour_spots];	
	
    int idx = 0;
	PTZ_TOUR_SPOT_T *tsp = NULL;

	for( int i=1;i<=tour->size_of_tour_spots;i++)
	{
		tsp = &(tour->tour_spots[idx++]);
		char tmpStr[20] = {0,};
		sprintf(tmpStr,"Preset%d.number",i);
		int presetId = std::stoi(tour_map[tmpStr].c_str());
	    strncpy( tsp->preset_alias, strdup(get_preset_name_by_id(presetId).c_str()), sizeof(tsp->preset_alias) ); 
		tsp->preset_numberic_alias = presetId;
		tsp->pan = 0;
		tsp->tilt = 0;
		tsp->zoom = 0;
		
		sprintf(tmpStr,"Preset%d.speed",i);
		float speed = std::stof(tour_map[tmpStr].c_str());
		float real_speed	= get_speed_quasi_sensitive_value( speed );
		tsp->speed = real_speed;
		sprintf(tmpStr,"Preset%d.Dwell time",i);
		int stay_time = std::stoi(tour_map[tmpStr].c_str());
		tsp->stay_time = stay_time*1000;	
	}	

	return VMS_PTZ_SUCCESS;
}

std::string samsung_ptz_controller::get_preset_name_by_id(int preset_id)
{	
	
	std::map<std::string,int>::iterator it;

	for(it = _preset_map.begin();it!=_preset_map.end();it++)
	{
		if(it->second == preset_id)	return it->first;
...
void samsung_ptz_controller::split2vector( std::string origin, std::string token, std::vector<std::string> *devided )
{
    int cut_at;  // 자르는 위치
    int index = 0;  // 문자열 인덱스
...
void samsung_ptz_controller::split2map( std::string origin, std::string token, std::map<std::string,std::string> *devided )
{
    int cut_at;  // 자르는 위치
    int index = 0;  // 문자열 인덱스
