...
class http_client;
#if defined(WIN32)
class __declspec(dllexport) truen_ptz_controller : public base_ptz_controller
#else
class truen_ptz_controller : public base_ptz_controller
#endif
{
public:

	truen_ptz_controller( void );
	~truen_ptz_controller( void );
	...
	unsigned short	relative_move( PTZ_RELATIVE_MOVE_TYPE_T move, float sensitive, float speed=-1 );
	unsigned short	relative_move( float pan_sensitive, float tilt_sensitive, float zoom_sensitive, float speed=-1 );
	unsigned short	absolute_move( float pan_sensitive, float tilt_sensitive, float zoom_sensitive , float speed=100 );
...
private:
	...
	float	get_re_pan_sensitive_value(float sensitive );
	float	get_re_tilt_sensitive_value( float sensitive);
	float	get_re_zoom_sensitive_value( float sensitive );
	float	get_re_sensitive_value( float sensitive );
	
	float	get_ab_pan_sensitive_value( float sensitive );
	float	get_ab_tilt_sensitive_value(float sensitive);
	float	get_ab_zoom_sensitive_value( float sensitive );
	...
	void		get_preset_list_mapped( void );
