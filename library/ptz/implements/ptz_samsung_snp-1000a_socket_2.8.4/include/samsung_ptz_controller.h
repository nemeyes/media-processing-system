...
#if defined(WIN32)
class __declspec(dllexport) samsung_ptz_controller : public base_ptz_controller, public socket_client
#else
...
private:
	void			login( void );
	void			step_phase_1( void );
	void			step_phase_2( void );
...
private:
	...
	void            set_cmd_data(CTRL* con, std::string cmd, int data=0);
