...
bool http_client::post_request( char *user_id, char *user_password, char *send_buffer, char **recv_buffer )
{
	bool result = false;
	CURLcode curl_result;
	...
	curl_easy_setopt( _curl, CURLOPT_PASSWORD, user_password );
	char cookies[50] = {0,};
    sprintf(cookies, "C_NAME=%s; C_PW=%s; C_VIDEO_AUTH=1", user_id, user_password);
