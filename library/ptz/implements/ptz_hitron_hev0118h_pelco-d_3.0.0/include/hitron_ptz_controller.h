...
class http_client;
#if defined(WIN32)
class __declspec(dllexport) hitron_ptz_controller : public base_ptz_controller
#else
class hitron_ptz_controller : public base_ptz_controller
#endif
{
public:
	hitron_ptz_controller( void );
	~hitron_ptz_controller( void );
...
private:
	...
	unsigned short  get_preset_list_map( std::map<std::string,std::string> *preset_map );	
