...
{
	return VMS_PTZ_DEVICE_INFO[AXIS_P214PTZ_VAPIX_V3_4_49][VENDOR];
}

char* axis_ptz_controller::get_vendor_device_name( void )
{
	return VMS_PTZ_DEVICE_INFO[AXIS_P214PTZ_VAPIX_V3_4_49][DEVICE];
}

char* axis_ptz_controller::get_vendor_device_protocol_name( void )
{
	return VMS_PTZ_DEVICE_INFO[AXIS_P214PTZ_VAPIX_V3_4_49][PROTOCOL];
}

char* axis_ptz_controller::get_vendor_device_version_name( void )
{
	return VMS_PTZ_DEVICE_INFO[AXIS_P214PTZ_VAPIX_V3_4_49][VERSION];
}

unsigned short axis_ptz_controller::get_vendor_id( void )
{

	return VMS_PTZ_DEVICE_ID[AXIS_P214PTZ_VAPIX_V3_4_49][VENDOR];
}

unsigned short axis_ptz_controller::get_vendor_device_id( void )
{
	return VMS_PTZ_DEVICE_ID[AXIS_P214PTZ_VAPIX_V3_4_49][DEVICE];
}

unsigned short axis_ptz_controller::get_vendor_device_protocol_id( void )
{
	return VMS_PTZ_DEVICE_ID[AXIS_P214PTZ_VAPIX_V3_4_49][PROTOCOL];
}

unsigned short axis_ptz_controller::get_vendor_device_version_id( void )
{
	return VMS_PTZ_DEVICE_ID[AXIS_P214PTZ_VAPIX_V3_4_49][VERSION];
}
...
unsigned short axis_ptz_controller::get_preset_list_map( std::map<std::string,std::string> *preset_map )
{
	...
	///axis-cgi/operator/param.cgi?action=list&group=PTZ.PresetPos.*.Label
	//=>root.PTZ.PresetPos.P0.Label=x
	http_client client( _host, _port_number, "/axis-cgi/operator/param.cgi" );
	client.put_variable( "action", "list" );
	client.put_variable( "group", "PTZ.PresetPos.*.Label" );
	...
	str_presets.append(presets);

	split2vector( str_presets, "\n", &vec );
	for( it=vec.begin(); it!=vec.end(); it++ )
	{
		std::string pair = *it;
		char preset_id[100] = {0,}; 
		char *tkn0, *preset_name;
		char* tgt = strdup(pair.c_str());

		tkn0 = strtok(tgt,"=");//root.PTZ.PresetPos.P0.Label=name1
		preset_name = strtok(NULL,"=");//name1
		tkn0 = strtok(tgt,".");//root
		tkn0 = strtok(NULL,".");//PTZ
		tkn0 = strtok(NULL,".");//PresetPos
		tkn0 = strtok(NULL,".");//P0
		sscanf(tkn0,"%*c%s",preset_id);
		
		preset_map->insert(std::make_pair(preset_id, preset_name) );
	}
...
unsigned short axis_ptz_controller::add_preset_tour( char *tour_name, int size )
{
	...
	///axis-cgi/operator/param.cgi?action=add&group=GuardTour&template=guardtour&GuardTour.G.Name=tour8  
	http_client client( _host, _port_number, "/axis-cgi/operator/param.cgi" );
...
{
	///axis-cgi/operator/param.cgi?action=remove&group=GuardTour.G1
	http_client client( _host, _port_number, "/axis-cgi/operator/param.cgi" );
...
{
	///axis-cgi/operator/param.cgi?action=remove&group=GuardTour.G1.Tour
	http_client client( _host, _port_number, "/axis-cgi/operator/param.cgi" );
	client.put_variable( "action", "remove" );	
	char str_group[20] = {0,};
...
{	
	...
	///axis-cgi/operator/param.cgi?action=update&root.GuardTour.G0.Running=no
	http_client client( _host, _port_number, "/axis-cgi/operator/param.cgi" );
...
{	
  ///axis-cgi/operator/param.cgi?action=add&group=GuardTour.G0.Tour&template=tour&GuardTour.G0.Tour.T.PresetNbr=1&GuardTour.G0.Tour.T.Position=2&GuardTour.G0.Tour.T.MoveSpeed=90&GuardTour.G0.Tour.T.WaitTime=10&GuardTour.G0.Tour.T.WaitTimeViewType=Seconds 
	std::map<std::string,std::string> preset_map;
...
{	
	query_limits();
	float max_val = _max_pan - _min_pan;
	
	if( _pan_min==_min_pan && _pan_max==_max_pan && abs(sensitive)<=abs(_pan_max-_pan_min))
		return sensitive;
...
{
	query_limits();
	float max_val = _max_tilt - _min_tilt;

	if( _tilt_min==_min_tilt && _tilt_max==_max_tilt && abs(sensitive)<=abs(_tilt_max-_tilt_min))
		return sensitive;
...
{
	query_limits();
	float max_val = _max_zoom - _min_zoom;

	if( _zoom_min==_min_zoom && _zoom_max==_max_zoom && abs(sensitive)<=abs(_zoom_max-_zoom_min))
		return sensitive;
...
float axis_ptz_controller::get_apan_sensitive_value( float sensitive )
{
	...
	if( _pan_min==_min_pan && _pan_max==_max_pan && sensitive>=_pan_min && sensitive<=_pan_max)
		return sensitive;
...
float axis_ptz_controller::get_atilt_sensitive_value( float sensitive )
{
	...
	if( _tilt_min==_min_tilt && _tilt_max==_max_tilt && sensitive>=_tilt_min && sensitive<=_tilt_max)
		return sensitive;
...
float axis_ptz_controller::get_azoom_sensitive_value( float sensitive )
{
	...
	if( _zoom_min==_min_zoom && _zoom_max==_max_zoom && sensitive>=_zoom_min && sensitive<=_zoom_max)
		return sensitive;
...
float axis_ptz_controller::get_apan_quasi_sensitive_value( float real_sensitive )
{
	...
	if( _pan_min==_min_pan && _pan_max==_max_pan && real_sensitive>=_min_pan && real_sensitive<=_max_pan)
		return real_sensitive;
...
float axis_ptz_controller::get_atilt_quasi_sensitive_value( float real_sensitive )
{
	...
	if( _tilt_min==_min_tilt && _tilt_max==_max_tilt && real_sensitive>=_min_tilt && real_sensitive<=_max_tilt)
		return real_sensitive;
...
float axis_ptz_controller::get_azoom_quasi_sensitive_value( float real_sensitive )
{
	...
	if( _zoom_min==_min_zoom && _zoom_max==_max_zoom && real_sensitive>=_min_zoom && real_sensitive<=_max_zoom)
		return real_sensitive;
