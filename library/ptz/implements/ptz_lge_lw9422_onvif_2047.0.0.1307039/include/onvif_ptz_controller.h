...
#include "gsoap/ptzMediaBindingProxy.h"
...
private:
	...
	SOAP_CLIENT_FUNC( GetProfile, onvif_ptz_controller, MediaBindingProxy, char*, _trt__GetProfile*, _trt__GetProfileResponse* )
	SOAP_CLIENT_FUNC( AddPTZConfiguration, onvif_ptz_controller, MediaBindingProxy, char*, _trt__AddPTZConfiguration*, _trt__AddPTZConfigurationResponse* );

	unsigned short	query_limits( void );
	unsigned short	query_status( float &pan, float &tilt, float &zoom );

	void			get_preset_list( void );
	void			get_preset_tour_list( void );
	...
	void			add_ptz_configuration_to_profile( void );
...
private:
	...
	bool				_is_added_ptz_configuration;
	bool				_is_inverse;
	std::map< std::string, std::string > _preset_list;
