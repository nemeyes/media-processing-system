...
{
	return VMS_PTZ_DEVICE_INFO[LGE_LW9422_ONVIF_2047_0_0_1307039][VENDOR];
}

char* onvif_ptz_controller::get_vendor_device_name( void )
{
	return VMS_PTZ_DEVICE_INFO[LGE_LW9422_ONVIF_2047_0_0_1307039][DEVICE];
}

char* onvif_ptz_controller::get_vendor_device_protocol_name( void )
{
	return VMS_PTZ_DEVICE_INFO[LGE_LW9422_ONVIF_2047_0_0_1307039][PROTOCOL];
}

char* onvif_ptz_controller::get_vendor_device_version_name( void )
{
	return VMS_PTZ_DEVICE_INFO[LGE_LW9422_ONVIF_2047_0_0_1307039][VERSION];
}

unsigned short onvif_ptz_controller::get_vendor_id( void )
{
	return VMS_PTZ_DEVICE_ID[LGE_LW9422_ONVIF_2047_0_0_1307039][VENDOR];
}

unsigned short onvif_ptz_controller::get_vendor_device_id( void )
{
	return VMS_PTZ_DEVICE_ID[LGE_LW9422_ONVIF_2047_0_0_1307039][DEVICE];
}

unsigned short onvif_ptz_controller::get_vendor_device_protocol_id( void )
{
	return VMS_PTZ_DEVICE_ID[LGE_LW9422_ONVIF_2047_0_0_1307039][PROTOCOL];
}

unsigned short onvif_ptz_controller::get_vendor_device_version_id( void )
{
	return VMS_PTZ_DEVICE_ID[LGE_LW9422_ONVIF_2047_0_0_1307039][VERSION];
}
...
unsigned short onvif_ptz_controller::set_preset_tour( PTZ_TOUR_T *tour )
{
	...
	request.PresetTour->StartingCondition->RecurringDuration	= soap_new_char( &proxy );
	...
	if( tour->size_of_tour_spots>0 )
	{
		...
		for( int index=0; index<tour->size_of_tour_spots; index++ )
		{
			...
			request.PresetTour->TourSpot[index]->StayTime			= soap_new_char( &proxy );
	...
	PTZBindingProxy proxy;
		...
		if( result==SOAP_OK )
		{
			for( int index=0; index<response.__sizePresetTour; index++ )
				_tour_list[response.PresetTour[index]->Name] = response.PresetTour[index]->Name;
			return VMS_PTZ_SUCCESS;
		}
	return VMS_PTZ_FAIL;
...
void onvif_ptz_controller::get_preset_tour_list( void )
{
	if( _tour_list.size()<1 )
	{
		PTZBindingProxy proxy;
		_tptz__GetPresetTours request;
		_tptz__GetPresetToursResponse response;
		...
		{
			for( int index=0; index<response.__sizePresetTour; index++ )
				_tour_list[response.PresetTour[index]->token] = response.PresetTour[index]->Name;
		}
	}
}
...
float onvif_ptz_controller::get_speed_sensitive_value( float sensitive )
{
	...
	{
		float real_sensitive = float(sensitive*(_max_speed-_min_speed))/float(_speed_max-_speed_min)/2;
...
float onvif_ptz_controller::get_continuous_sensitive_value( float sensitive )
{
	...
	if( abs(sensitive)<_speed_min ) 
		return _min_speed;
	else if( abs(sensitive)>_speed_max ) 
		return _max_speed;
	else
	{
		float real_sensitive = float(sensitive*(_max_speed-_min_speed))/float(_speed_max-_speed_min)/2;
