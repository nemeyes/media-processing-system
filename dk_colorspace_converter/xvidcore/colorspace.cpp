...
MAKE_COLORSPACE(rgb555_to_yv12, 2, 2, 2, RGB16_TO_YV12, MK_RGB555, 0, 0, 0)
MAKE_COLORSPACE(rgb565_to_yv12, 2, 2, 2, RGB16_TO_YV12, MK_RGB565, 0, 0, 0)
MAKE_COLORSPACE(bgr_to_yv12, 3, 2, 2, RGB_TO_YV12, 2, 1, 0, 0)
MAKE_COLORSPACE(bgra_to_yv12, 4, 2, 2, RGB_TO_YV12, 2, 1, 0, 0)
MAKE_COLORSPACE(rgb_to_yv12, 3, 2, 2, RGB_TO_YV12, 0, 1, 2, 0)
MAKE_COLORSPACE(abgr_to_yv12, 4, 2, 2, RGB_TO_YV12, 3, 2, 1, 0)
MAKE_COLORSPACE(rgba_to_yv12, 4, 2, 2, RGB_TO_YV12, 0, 1, 2, 0)
MAKE_COLORSPACE(argb_to_yv12, 4, 2, 2, RGB_TO_YV12, 1, 2, 3, 0)
MAKE_COLORSPACE(yuyv_to_yv12, 2, 2, 2, YUYV_TO_YV12, 0, 1, 2, 3)
MAKE_COLORSPACE(uyvy_to_yv12, 2, 2, 2, YUYV_TO_YV12, 1, 0, 3, 2)

MAKE_COLORSPACE(rgb555i_to_yv12, 2, 2, 4, RGB16I_TO_YV12, MK_RGB555, 0, 0, 0)
MAKE_COLORSPACE(rgb565i_to_yv12, 2, 2, 4, RGB16I_TO_YV12, MK_RGB565, 0, 0, 0)
MAKE_COLORSPACE(bgri_to_yv12, 3, 2, 4, RGBI_TO_YV12, 2, 1, 0, 0)
MAKE_COLORSPACE(bgrai_to_yv12, 4, 2, 4, RGBI_TO_YV12, 2, 1, 0, 0)
MAKE_COLORSPACE(abgri_to_yv12, 4, 2, 4, RGBI_TO_YV12, 3, 2, 1, 0)
MAKE_COLORSPACE(rgbi_to_yv12, 3, 2, 4, RGBI_TO_YV12, 0, 1, 2, 0)
MAKE_COLORSPACE(rgbai_to_yv12, 4, 2, 4, RGBI_TO_YV12, 0, 1, 2, 0)
MAKE_COLORSPACE(argbi_to_yv12, 4, 2, 4, RGBI_TO_YV12, 1, 2, 3, 0)
MAKE_COLORSPACE(yuyvi_to_yv12, 2, 2, 4, YUYVI_TO_YV12, 0, 1, 2, 3)
MAKE_COLORSPACE(uyvyi_to_yv12, 2, 2, 4, YUYVI_TO_YV12, 1, 0, 3, 2)
...
#define YV12_TO_RGB_ROW(SIZE,C1,C2,C3,C4)
...
#define YV12_TO_RGBI_ROW(SIZE,C1,C2,C3,C4)
...
MAKE_COLORSPACE(yv12_to_rgb555, 2, 2, 2, YV12_TO_RGB16, MK_RGB555, 0, 0, 0)
MAKE_COLORSPACE(yv12_to_rgb565, 2, 2, 2, YV12_TO_RGB16, MK_RGB565, 0, 0, 0)
//MAKE_COLORSPACE(yv12_to_bgr, 3, 2, 2, YV12_TO_RGB, 2, 1, 0, 0)
...
void yv12_to_bgr(uint8_t * x_ptr, int x_stride,	
				 uint8_t * y_ptr, uint8_t * u_ptr, uint8_t * v_ptr,	
				 int y_stride, int uv_stride,	
				 int width, int height, int vflip)	
{	
	...
	{			
		//YV12_TO_RGB_ROW(3,2,1,0,0);				
		for (x = 0; x < fixed_width; x+=(2)) 
		{	
			//YV12_TO_RGB(3,2,1,0,0);				
			//#define YV12_TO_RGB(SIZE,C1,C2,C3,C4)	
			int rgb_y;
			int b_u0 = B_U_tab[u_ptr[0]];
			int g_uv0 = G_U_tab[u_ptr[0]] + G_V_tab[v_ptr[0]];
			int r_v0 = R_V_tab[v_ptr[0]];
			WRITE_RGB(3, 0, 0, 2, 1, 0, 0);
			//WRITE_RGB(3, 1, 0, 2, 1, 0, 0);


//#define WRITE_RGB(SIZE,ROW,UV_ROW,C1,C2,C3,C4)
		rgb_y = RGB_Y_tab[ y_ptr[(1)*y_stride + 0] ];
		x_ptr[(1)*x_stride+(0)] = MAX(0, MIN(255, (rgb_y + b_u0) >> SCALEBITS_OUT));
		x_ptr[(1)*x_stride+(1)] = MAX(0, MIN(255, (rgb_y - g_uv0) >> SCALEBITS_OUT));
		x_ptr[(1)*x_stride+(2)] = MAX(0, MIN(255, (rgb_y + r_v0) >> SCALEBITS_OUT));
		//if ((3)>3) x_ptr[(ROW)*x_stride+(C4)] = 0;									
		rgb_y = RGB_Y_tab[ y_ptr[(1)*y_stride + 1] ];									
		x_ptr[(1)*x_stride+(3)+(0)] = MAX(0, MIN(255, (rgb_y + b_u0) >> SCALEBITS_OUT));	
		x_ptr[(1)*x_stride + (3) + (1)] = MAX(0, MIN(255, (rgb_y - g_uv0) >> SCALEBITS_OUT));
		x_ptr[(1)*x_stride + (3) + (2)] = MAX(0, MIN(255, (rgb_y + r_v0) >> SCALEBITS_OUT));
		//if ((3)>3) x_ptr[(ROW)*x_stride+(SIZE)+(C4)] = 0;
...
MAKE_COLORSPACE(yv12_to_bgra, 4, 2, 2, YV12_TO_RGB, 2, 1, 0, 3)
MAKE_COLORSPACE(yv12_to_abgr, 4, 2, 2, YV12_TO_RGB, 3, 2, 1, 0)
MAKE_COLORSPACE(yv12_to_rgb, 3, 2, 2, YV12_TO_RGB, 0, 1, 2, 0)
MAKE_COLORSPACE(yv12_to_rgba, 4, 2, 2, YV12_TO_RGB, 0, 1, 2, 3)
MAKE_COLORSPACE(yv12_to_argb, 4, 2, 2, YV12_TO_RGB, 1, 2, 3, 0)
MAKE_COLORSPACE(yv12_to_yuyv, 2, 2, 2, YV12_TO_YUYV, 0, 1, 2, 3)
MAKE_COLORSPACE(yv12_to_uyvy, 2, 2, 2, YV12_TO_YUYV, 1, 0, 3, 2)

MAKE_COLORSPACE(yv12_to_rgb555i, 2, 2, 4, YV12_TO_RGB16I, MK_RGB555, 0, 0, 0)
MAKE_COLORSPACE(yv12_to_rgb565i, 2, 2, 4, YV12_TO_RGB16I, MK_RGB565, 0, 0, 0)
MAKE_COLORSPACE(yv12_to_bgri, 3, 2, 4, YV12_TO_RGBI, 2, 1, 0, 0)
MAKE_COLORSPACE(yv12_to_bgrai, 4, 2, 4, YV12_TO_RGBI, 2, 1, 0, 3)
MAKE_COLORSPACE(yv12_to_abgri, 4, 2, 4, YV12_TO_RGBI, 3, 2, 1, 0)
MAKE_COLORSPACE(yv12_to_rgbi, 3, 2, 4, YV12_TO_RGBI, 0, 1, 2, 0)
MAKE_COLORSPACE(yv12_to_rgbai, 4, 2, 4, YV12_TO_RGBI, 0, 1, 2, 3)
MAKE_COLORSPACE(yv12_to_argbi, 4, 2, 4, YV12_TO_RGBI, 1, 2, 3, 0)
MAKE_COLORSPACE(yv12_to_yuyvi, 2, 2, 4, YV12_TO_YUYVI, 0, 1, 2, 3)
MAKE_COLORSPACE(yv12_to_uyvyi, 2, 2, 4, YV12_TO_YUYVI, 1, 0, 3, 2)

void yv12_to_yv12(uint8_t * y_dst, uint8_t * u_dst, uint8_t * v_dst,
