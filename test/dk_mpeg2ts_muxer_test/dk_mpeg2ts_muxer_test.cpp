...
static void log_packet(const AVFormatContext *fmt_ctx, const AVPacket *pkt)
{
	...
	char string[AV_TS_MAX_STRING_SIZE] = { 0 };
	

	printf("pts:%s pts_time:%s dts:%s dts_time:%s duration:%s duration_time:%s stream_index:%d\n",
		av_ts_make_string(string, pkt->pts), av_ts_make_time_string(string, pkt->pts, time_base),
		av_ts_make_string(string, pkt->pts), av_ts_make_time_string(string, pkt->dts, time_base),
		av_ts_make_string(string, pkt->duration), av_ts_make_time_string(string, pkt->duration, time_base), pkt->stream_index);
}
...
static void open_audio(AVFormatContext *oc, AVCodec *codec, OutputStream *ost, AVDictionary *opt_arg)
{
	...
	av_dict_free(&opt);
	if (ret < 0) {
		char str_error[AV_ERROR_MAX_STRING_SIZE] = { 0 };
		fprintf(stderr, "Could not open audio codec: %s\n", av_make_error_string(str_error, AV_ERROR_MAX_STRING_SIZE, ret));
		exit(1);
	}
...
static AVFrame *get_audio_frame(OutputStream *ost)
{
	...
	AVRational avRational = { 1, 1 };
	if (av_compare_ts(ost->next_pts, ost->st->codec->time_base,
		STREAM_DURATION, avRational) >= 0)
		return NULL;
...
static int write_audio_frame(AVFormatContext *oc, OutputStream *ost)
{
	...
	if (frame) {
		...
		*/
		...
		AVRational avRational = { 1, c->sample_rate };
		frame->pts = av_rescale_q(ost->samples_count, avRational, c->time_base);
...
int main(int argc, char **argv)
{
	...
	* video codecs and allocate the necessary encode buffers. */
	...
	if (!(fmt->flags & AVFMT_NOFILE)) {
		...
		if (ret < 0) {
			char str_error[AV_ERROR_MAX_STRING_SIZE] = { 0 };
			fprintf(stderr, "Could not open '%s': %s\n", filename, av_make_error_string(str_error, AV_ERROR_MAX_STRING_SIZE, ret));
			return 1;
		}
	}
