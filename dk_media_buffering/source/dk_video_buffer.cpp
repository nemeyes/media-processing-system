...
dk_media_buffering::ERR_CODE dk_video_buffer::push_bitstream(uint8_t * bs, size_t size)
{
	dk_video_auto_lock lock(&_mutex);
	return push(bs, size);
}

dk_media_buffering::ERR_CODE dk_video_buffer::pop_bitstream(uint8_t * bs, size_t & size)
{
	dk_media_buffering::ERR_CODE status = dk_media_buffering::ERR_CODE_FAILED;
	dk_video_auto_lock lock(&_mutex);
	status = pop(bs, size);
	return status;
}
...
dk_media_buffering::ERR_CODE dk_video_buffer::push(uint8_t * bs, size_t size)
{
	...
	if (bs && size > 0)
	{
		...
		vbuffer->amount = size;
		int32_t result = dk_circular_buffer_write(_cbuffer, bs, vbuffer->amount);
		if (result == -1)
		{
			if (vbuffer->prev)
...
dk_media_buffering::ERR_CODE dk_video_buffer::pop(uint8_t * bs, size_t & size)
{
	dk_media_buffering::ERR_CODE status = dk_media_buffering::ERR_CODE_SUCCESS;
	size = 0;
	vbuffer_t * vbuffer = _root->next;
	if (vbuffer)
	{
		...
		int32_t result = dk_circular_buffer_read(_cbuffer, bs, vbuffer->amount);
		if (result == -1)
			status = dk_media_buffering::ERR_CODE_FAILED;
