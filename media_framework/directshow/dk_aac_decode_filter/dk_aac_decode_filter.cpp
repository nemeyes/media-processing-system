...
HRESULT dk_aac_decode_filter::Transform(IMediaSample *src, IMediaSample *dst)
{
	...
	dk_aac_decoder::dk_audio_entity_t encoded;
	encoded.data = input_buffer;
	encoded.data_size = input_data_size;
	encoded.data_capacity = input_data_size;
	dk_aac_decoder::dk_audio_entity_t pcm;// = { output_buffer, 0, output_data_size };
	pcm.data = output_buffer;
	pcm.data_capacity = output_data_size;
	pcm.data_size = 0;
