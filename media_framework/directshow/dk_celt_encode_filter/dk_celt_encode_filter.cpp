...
HRESULT dk_celt_encode_filter::Receive(IMediaSample *pSample)
{
	...
	dk_celt_encoder::dk_audio_entity_t pcm;
	pcm.data = inbuffer;
	pcm.data_size = insize;
	pcm.data_capacity = insize;
	dk_celt_encoder::err_code ret = _encoder->encode(&pcm);
	if (ret == dk_celt_encoder::err_code_success)
	{
		while (1)
		{
			dk_celt_encoder::dk_audio_entity_t encoded;// = { outbuffer, 0, outsize };
			encoded.data = outbuffer;
			encoded.data_size = 0;
			encoded.data_capacity = outsize;
