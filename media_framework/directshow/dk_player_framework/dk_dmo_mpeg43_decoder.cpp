...
CComPtr<IPin> dk_dmo_mpeg43_decoder::get_output_pin(void)
{
	CComPtr<IPin> outpin;
...
CComPtr<IPin> dk_dmo_mpeg43_decoder::get_input_pin(void)
{
	CComPtr<IPin> inpin;
...
HRESULT dk_dmo_mpeg43_decoder::add_to_graph(CComPtr<IGraphBuilder> graph)
{
	...
	if (_found)
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				if (SUCCEEDED(hr))
				{
					hr = graph->AddFilter(decoder, L"Mpeg43 Decoder DMO");
...
void dk_dmo_mpeg43_decoder::enum_decoder_dmo(void)
{
	_found = FALSE;
	IEnumDMO* pEnum = NULL;
	...
	if (SUCCEEDED(hr))
	{
		...
		do
		{
			...
			if (hr == S_OK)
			{
				...
				if (wcscmp(wszName, L"Mpeg43 Decoder DMO") == 0)
				{
					_clsidDMO = clsidDMO;
					_found = TRUE;
				}
