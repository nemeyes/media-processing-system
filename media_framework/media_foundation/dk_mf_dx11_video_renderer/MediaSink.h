...
namespace DX11VideoRenderer
{
	class CMediaSink :
		...
		public IKeyEvent,
		public IGPUHandler,
		public IVideoCodecConfigurator,
		public IMediaStreamerConfigurator,
		private CBase
	{
	public:
		...
		STDMETHODIMP SetVideoCodec(INT vcodec);
		STDMETHODIMP SetVideoWidth(INT width);
		STDMETHODIMP SetVideoHeight(INT height);
		STDMETHODIMP SetVideoBitrate(INT bitrate);
		STDMETHODIMP SetVideoFPS(INT fps);
		STDMETHODIMP SetVideoKeyframeInterval(INT interval);

		// ICAPStreamerConfigurator
		STDMETHODIMP SetUUID(WCHAR * uuid);
		STDMETHODIMP SetAddress(WCHAR * address);
		STDMETHODIMP SetPortNumber(INT port_number);
