...
#include <dk_video_codec_configurator.h>
#include <dk_media_streamer_configurator.h>
...
namespace DX11VideoRenderer
{
	...
	public:
		...
		void EnablePresent(BOOL enable);

		// Video Codec Configurator
		void SetVideoCodec(INT vcodec);
		void SetVideoWidth(INT width);
		void SetVideoHeight(INT height);
		void SetVideoBitrate(INT bitrate);
		void SetVideoFPS(INT fps);
		void SetVideoKeyframeInterval(INT interval);

		// Streamer Configurator
		void SetUUID(WCHAR * uuid);
		void SetAddress(WCHAR * address);
	...
	private:
		...
		INT								_vcodec_bitrate;
		INT								_vcodec_fps;
		INT								_vcodec_keyframe_interval;
		wchar_t							_streamer_uuid[MAX_PATH];
		wchar_t							_streamer_address[MAX_PATH];
		INT								_streamer_port_number;
		...
		ID3D11Buffer* m_pSphereVertBuffer;
		...
		cap_nvenc_encoder::configuration_t _nvenc_encoder_configuration;
		cap_nvenc_encoder * _nvenc_encoder;

#if defined(WITH_BLOCK_FUNCTION_VER1) || defined(WITH_BLOCK_FUNCTION_VER2)
		cap_rtmp_client * _rtmp_streamer;
#endif
#endif
		/*
		//DWrite
		...
		ID3D11ShaderResourceView* m_pD2DTexture;
		std::wstring m_printText;
		float m_fWidth;
		float m_fHeight;
		ID3D11BlendState* m_pTransparency;
