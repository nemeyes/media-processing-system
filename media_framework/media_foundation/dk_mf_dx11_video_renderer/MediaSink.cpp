...
HRESULT DX11VideoRenderer::CMediaSink::QueryInterface(REFIID iid, __RPC__deref_out _Result_nullonfailure_ void** ppv)
{
	...
	else if (iid == __uuidof(IVideoCodecConfigurator))
	{
		*ppv = static_cast<IVideoCodecConfigurator*>(this);
	}
	else if (iid == __uuidof(IMediaStreamerConfigurator))
	{
		*ppv = static_cast<IMediaStreamerConfigurator*>(this);
	}
	else
	{
		*ppv = NULL;
...
STDMETHODIMP DX11VideoRenderer::CMediaSink::EnablePresent(BOOL enable)
{
	if (m_pPresenter)
		m_pPresenter->EnablePresent(enable);
	return NOERROR;
}

// ICAPVideoCodecConfigurator
STDMETHODIMP DX11VideoRenderer::CMediaSink::SetVideoCodec(INT vcodec)
{
	if (m_pPresenter)
		m_pPresenter->SetVideoCodec(vcodec);
	return NOERROR;
}

STDMETHODIMP DX11VideoRenderer::CMediaSink::SetVideoWidth(INT width)
{
	if (m_pPresenter)
		m_pPresenter->SetVideoWidth(width);
	return NOERROR;
}

STDMETHODIMP DX11VideoRenderer::CMediaSink::SetVideoHeight(INT height)
{
	if (m_pPresenter)
		m_pPresenter->SetVideoHeight(height);
	return NOERROR;
}

STDMETHODIMP DX11VideoRenderer::CMediaSink::SetVideoBitrate(INT bitrate)
{
	if (m_pPresenter)
		m_pPresenter->SetVideoBitrate(bitrate);
	return NOERROR;
}

STDMETHODIMP DX11VideoRenderer::CMediaSink::SetVideoFPS(INT fps)
{
	if (m_pPresenter)
		m_pPresenter->SetVideoFPS(fps);
	return NOERROR;
}

STDMETHODIMP DX11VideoRenderer::CMediaSink::SetVideoKeyframeInterval(INT interval)
{
	if (m_pPresenter)
		m_pPresenter->SetVideoKeyframeInterval(interval);
	return NOERROR;
}

STDMETHODIMP DX11VideoRenderer::CMediaSink::SetUUID(WCHAR * uuid)
{
	if (m_pPresenter)
		m_pPresenter->SetUUID(uuid);
	return NOERROR;
}

STDMETHODIMP DX11VideoRenderer::CMediaSink::SetAddress(WCHAR * address)
{
	if (m_pPresenter)
		m_pPresenter->SetAddress(address);
	return NOERROR;
}

STDMETHODIMP DX11VideoRenderer::CMediaSink::SetPortNumber(INT port_number)
{
	if (m_pPresenter)
		m_pPresenter->SetPortNumber(port_number);
	return NOERROR;
}
