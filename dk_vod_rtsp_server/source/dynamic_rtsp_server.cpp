#include "dynamic_rtsp_server.h"
#include <liveMedia.hh>
#include <string.h>

dynamic_rtsp_server * dynamic_rtsp_server::createNew(UsageEnvironment& env, Port ourPort, UserAuthenticationDatabase* authDatabase, unsigned reclamationTestSeconds)
{
	int ourSocket = setUpOurSocket(env, ourPort);
	if (ourSocket == -1)
		return NULL;
	return new dynamic_rtsp_server(env, ourSocket, ourPort, authDatabase, reclamationTestSeconds);
}

dynamic_rtsp_server::dynamic_rtsp_server(UsageEnvironment& env, int ourSocket, Port ourPort, UserAuthenticationDatabase* authDatabase, unsigned reclamationTestSeconds)
	: RTSPServerSupportingHTTPStreaming(env, ourSocket, ourPort, authDatabase, reclamationTestSeconds)
{
}

dynamic_rtsp_server::~dynamic_rtsp_server()
{
}

static ServerMediaSession* createNewSMS(UsageEnvironment& env, char const* fileName, FILE* fid); // forward

ServerMediaSession * dynamic_rtsp_server::lookupServerMediaSession(char const* streamName, Boolean isFirstLookupInSession)
{
