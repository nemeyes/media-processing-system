	...
	, _pps(0)
{
	...
	_kill_trigger = envir().taskScheduler().createEventTrigger((TaskFunc*)&(live_media_wrapper::kill_trigger));
	//_task_sched = &_env->taskScheduler();
	if (username && password && strlen(username)>0 && strlen(password)>0)
        _auth = new Authenticator(username, password);
}

live_media_wrapper::~live_media_wrapper(void)
{
	if (_auth)
	{
		delete _auth;
...
void live_media_wrapper::get_options(RTSPClient::responseHandler * after_func)
{
    sendOptionsCommand(after_func, _auth);
}

void live_media_wrapper::get_description(RTSPClient::responseHandler * after_func)
{
    sendDescribeCommand(after_func, _auth);
}

void live_media_wrapper::setup_media_subsession(MediaSubsession * media_subsession, bool rtp_over_tcp, bool force_multicast_unspecified, RTSPClient::responseHandler * after_func)
{
    sendSetupCommand(*media_subsession, after_func, False, rtp_over_tcp?True:False, force_multicast_unspecified?True:False, _auth);
}

void live_media_wrapper::start_playing_session(MediaSession * media_session, double start, double end, float scale, RTSPClient::responseHandler * after_func)
{
    sendPlayCommand(*media_session, after_func, start, end, scale, _auth);
}

void live_media_wrapper::start_playing_session(MediaSession * media_session, const char * abs_start_time, const char * abs_end_time, float scale, RTSPClient::responseHandler * after_func)
{
    sendPlayCommand(*media_session, after_func, abs_start_time, abs_end_time, scale, _auth);
}

void live_media_wrapper::teardown_session(MediaSession * media_session, RTSPClient::responseHandler * after_func)
{
    sendTeardownCommand(*media_session, after_func, _auth);
}

void live_media_wrapper::set_user_agent_string(const char * user_agent)
{
...
void live_media_wrapper::continue_after_options(RTSPClient * param, int result_code, char * result_string)
{
    live_media_wrapper * self = static_cast<live_media_wrapper*>(param);
...
void live_media_wrapper::continue_after_describe(RTSPClient * param, int result_code, char * result_string)
{
    live_media_wrapper * self = static_cast<live_media_wrapper*>(param);
	...
	do
	{
		...
		char medium[10] = { 0 };
		if (self->_recv_option == dk_rtsp_client::RECV_AUDIO)
			strncpy(medium, "audio", sizeof(medium));
		if (self->_recv_option == dk_rtsp_client::RECV_VIDEO)
...
void live_media_wrapper::continue_after_play( RTSPClient * param, int result_code, char * result_string )
{
    live_media_wrapper * self = static_cast<live_media_wrapper*>(param);
...
void live_media_wrapper::close(void)
{
	envir().taskScheduler().triggerEvent(_kill_trigger, this);
}

void live_media_wrapper::subsession_after_playing( void * param )
{
	MediaSubsession * media_subsession = static_cast<MediaSubsession*>(param);
	live_media_wrapper * self = static_cast<live_media_wrapper*>(media_subsession->miscPtr);
...
void live_media_wrapper::subsession_bye_handler( void * param )
{
	MediaSubsession * media_subsession = static_cast<MediaSubsession*>(param);
	live_media_wrapper * self = static_cast<live_media_wrapper*>(media_subsession->miscPtr);
...
void live_media_wrapper::setup_streams( void )
{
	...
	while ((media_subsession = _iter->next()) != 0)
    {
        if( media_subsession->clientPortNum()==0 )
            continue;
        if(_transport_option==dk_rtsp_client::RTP_OVER_TCP)
			setup_media_subsession(media_subsession, true, false, live_media_wrapper::continue_after_setup);
        else
			setup_media_subsession(media_subsession, false, false, live_media_wrapper::continue_after_setup);
...
void live_media_wrapper::shutdown(void)
{
    if( _shutting_down )
        return;
...
void live_media_wrapper::session_timer_handler( void * param )
{
    live_media_wrapper * self = static_cast<live_media_wrapper*>( param );
...
void live_media_wrapper::check_packet_arrival( void * param )
{
    live_media_wrapper * self = static_cast<live_media_wrapper*>( param );
    int delay_usec = 100000; //100 ms
    self->_arrival_check_timer_task = self->envir().taskScheduler().scheduleDelayedTask( delay_usec, (TaskFunc*)&live_media_wrapper::check_packet_arrival, self );
}

void live_media_wrapper::check_inter_packet_gaps( void * param )
{
    live_media_wrapper * self = static_cast<live_media_wrapper*>( param );
    ...
    else
    {
		...
		self->_inter_packet_gap_check_timer_task = self->envir().taskScheduler().scheduleDelayedTask(self->_inter_packet_gap_max_time * 1000000,
                                                                                                    (TaskFunc*)&live_media_wrapper::check_inter_packet_gaps, self );
    }
}

void live_media_wrapper::check_session_timeout_broken_server( void * param )
{
    live_media_wrapper * self = static_cast<live_media_wrapper*>( param );
	...
	self->_session_timeout_broken_server_task = self->envir().taskScheduler().scheduleDelayedTask(delay_sec_until_next_keepalive * 1000000,
                                                                                                 (TaskFunc*)&live_media_wrapper::check_session_timeout_broken_server, self );
}
