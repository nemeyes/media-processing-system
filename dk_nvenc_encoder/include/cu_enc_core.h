#ifndef _CU_ENC_CORE_H_
#define _CU_ENC_CORE_H_
...
private:
	...
	dk_nvenc_encoder::ERR_CODE process(const cu_enc_core::cu_enc_buffer_t * enc_buffer, unsigned char * encoded, unsigned int & encoded_size, NV_ENC_PIC_TYPE & bs_pic_type);
...
//    NVENCSTATUS NvEncReconfigureEncoder(const NvEncPictureCommand *pEncPicCommand);
	...
	NVENCSTATUS NvEncEncodeFrame(cu_enc_core::cu_enc_buffer_t * buffer, uint32_t width, uint32_t height);
...
private:
	...
	cu_enc_core::cu_enc_buffer_t _enc_buffer[MAX_ENCODE_QUEUE];
	cu_enc_core::cu_enc_queue<cu_enc_core::cu_enc_buffer_t> _enc_buffer_queue;
	cu_enc_core::enc_output_buffer_t _enc_eos_output_buffer;
