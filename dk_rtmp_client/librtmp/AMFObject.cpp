...
void RTMP_LIB::AMFObjectProperty::Dump() const
{
  ...
  char strRes[256]="";
  _snprintf(strRes, 255, "Name: %25s, ", m_strName.empty()? "no-name.":m_strName.c_str());

  char str[256]="";
  switch(m_type)
  {
    case AMF_NUMBER:
		_snprintf(str, 255, "NUMBER:\t%.2f", m_dNumVal);
      break;
    case AMF_BOOLEAN:
		_snprintf(str, 255, "BOOLEAN:\t%s", m_dNumVal == 1. ? "TRUE" : "FALSE");
      break;
    case AMF_STRING:
		_snprintf(str, 255, "STRING:\t%s", m_strVal.c_str());
      break;
    case AMF_DATE:
		_snprintf(str, 255, "DATE:\ttimestamp: %.2f, UTC offset: %d", m_dNumVal, m_nUTCOffset);
      break;
    default:
		_snprintf(str, 255, "INVALID TYPE 0x%02x", (unsigned char)m_type);
  }
