			...
			switch( prop.GetType() )
			{
				case AMF_NUMBER:
					if ( (double)int(prop.GetNumber()) == prop.GetNumber() )
						_snprintf(str, 255, "%.0f", prop.GetNumber() );
					else
						_snprintf(str, 255, "%.2f", prop.GetNumber());
					break;
				case AMF_BOOLEAN:
					_snprintf(str, 255, "%s", prop.GetNumber() == 1. ? "TRUE" : "FALSE");
					break;
				case AMF_STRING:
					_snprintf(str, 255, "%s", prop.GetString().c_str());
					break;
				case AMF_DATE:
					_snprintf(str, 255, "timestamp:%.2f", prop.GetNumber());
					break;
				default:
					_snprintf(str, 255, "INVALID TYPE 0x%02x", (unsigned char)prop.GetType());
			}
			if ( prop.GetPropName() != "" ) {
				// chomp
