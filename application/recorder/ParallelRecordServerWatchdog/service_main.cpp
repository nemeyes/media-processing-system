...
BOOL CreateProcessByName(CHAR *processName)
{
	...
	HINSTANCE self;
	self = ::GetModuleHandleA("ParallelRecordServerWatchdog.exe");
...
int _tmain(int argc, TCHAR *argv[])
{
	...
	{
		OutputDebugString(_T("DebuggerKing Parallel Record Server Service Returned Error"));
		return GetLastError();
	}
	return 0;
}
...
VOID WINAPI ServiceMain(DWORD argc, LPTSTR *argv)
{
	...
	OutputDebugString(_T("DebuggerKing Parallel Record Server Service : ServiceMain: Entry"));

	g_StatusHandle = RegisterServiceCtrlHandler(SERVICE_NAME, ServiceCtrlHandler);

	if (g_StatusHandle == NULL)
	{
		OutputDebugString(_T("DebuggerKing Parallel Record Server Service : ServiceMain: RegisterServiceCtrlHandler returned error"));
		goto EXIT;
	}
	...
	if (g_ServiceStopEvent == NULL)
	{
		...
		if (SetServiceStatus(g_StatusHandle, &g_ServiceStatus) == FALSE)
		{
			OutputDebugString(_T("DebuggerKing Parallel Record Server Service Returned Warning"));
		}
		goto EXIT;
	}
	...
	if (SetServiceStatus(g_StatusHandle, &g_ServiceStatus) == FALSE)
	{
		OutputDebugString(_T("DebuggerKing Parallel Record Server Service SetServiceStatus returned error"));
	}

	// Start the thread that will perform the main task of the service
	HANDLE hThread = CreateThread(NULL, 0, ServiceWorkerThread, NULL, 0, NULL);

	OutputDebugString(_T("DebuggerKing Parallel Record Server Service Waiting for Worker Thread to complete"));

	// Wait until our worker thread exits effectively signaling that the service needs to stop
	WaitForSingleObject(hThread, INFINITE);

	OutputDebugString(_T("DebuggerKing Parallel Record Server Service Worker Thread Stop Event signaled"));


	/*
	* Perform any cleanup tasks
	*/
	OutputDebugString(_T("DebuggerKing Parallel Record Server Service Performing Cleanup Operations"));
...
EXIT:
	OutputDebugString(_T("DebuggerKing Parallel Record Server Service Exit"));
...
{
	OutputDebugString(_T("DebuggerKing Parallel Record Server Service : ServiceCtrlHandler: Entry"));
	switch (CtrlCode)
	{
	case SERVICE_CONTROL_STOP:
	{
		OutputDebugString(_T("DebuggerKing Parallel Record Server Service : SERVICE_CONTROL_STOP Request"));
	...
	OutputDebugString(_T("DebuggerKing Parallel Record Server Service : ServiceCtrlHandler: Exit"));
}

DWORD WINAPI ServiceWorkerThread(LPVOID lpParam)
{
	CreateProcessByName("ParallelRecordServer.exe");
	while (WaitForSingleObject(g_ServiceStopEvent, 0) != WAIT_OBJECT_0)
	{
		if (!IsProcessRunning(_T("ParallelRecordServer.exe")))
		{
			killProcessByName(_T("ParallelRecordServer.exe"));
			CreateProcessByName("ParallelRecordServer.exe");
		}
		Sleep(1000);
	}
