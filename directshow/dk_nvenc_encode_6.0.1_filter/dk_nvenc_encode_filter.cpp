...
#include "dk_nvenc_encode_filter_properties.h"
#include "dk_nvenc_encode_filter.h"

dk_nvenc_encode_filter::dk_nvenc_encode_filter(LPUNKNOWN unk, HRESULT *hr)
	: CTransformFilter(g_szFilterName, unk, CLSID_DK_NVENC_ENCODE_FILTER)
{
	_encoder = new dk_nvenc_encoder();
}

dk_nvenc_encode_filter::~dk_nvenc_encode_filter(VOID)
{
	if (_encoder)
		delete _encoder;
}

CUnknown * WINAPI dk_nvenc_encode_filter::CreateInstance(LPUNKNOWN unk, HRESULT *hr)
{
	*hr = S_OK;
	CUnknown* punk = new dk_nvenc_encode_filter(unk, hr);
	if (punk == NULL)
		*hr = E_OUTOFMEMORY;
	return punk;
}

STDMETHODIMP dk_nvenc_encode_filter::NonDelegatingQueryInterface(REFIID riid, void** ppv)
{
...
HRESULT  dk_nvenc_encode_filter::StartStreaming()
{
	return NOERROR;
}

HRESULT  dk_nvenc_encode_filter::StopStreaming()
{
	return NOERROR;
}

// override this to grab extra interfaces on connection
HRESULT  dk_nvenc_encode_filter::CheckConnect(PIN_DIRECTION direction, IPin *pin)
{
...
HRESULT  dk_nvenc_encode_filter::BreakConnect(PIN_DIRECTION direction)
{
...
HRESULT  dk_nvenc_encode_filter::CompleteConnect(PIN_DIRECTION direction, IPin *pin)
{
	if (direction == PINDIR_INPUT)
	{
		if (_encoder)
...
HRESULT  dk_nvenc_encode_filter::AlterQuality(Quality quality)
{
	UNREFERENCED_PARAMETER(quality);
	return S_FALSE;
}

HRESULT  dk_nvenc_encode_filter::SetMediaType(PIN_DIRECTION direction, const CMediaType *type)
{
	UNREFERENCED_PARAMETER(direction);
...
HRESULT  dk_nvenc_encode_filter::EndOfStream(void)
{
	HRESULT hr = NOERROR;
	if (m_pOutput != NULL)
	{
...
HRESULT  dk_nvenc_encode_filter::BeginFlush(void)
{
	HRESULT hr = NOERROR;
	if (m_pOutput != NULL)
	{
...
// or a worker thread
HRESULT  dk_nvenc_encode_filter::EndFlush(void)
{
...
HRESULT  dk_nvenc_encode_filter::NewSegment(REFERENCE_TIME tStart, REFERENCE_TIME tStop, double dRate)
{
	if (m_pOutput != NULL)
	{
...
HRESULT  dk_nvenc_encode_filter::CheckInputType(const CMediaType *type)
{
...
HRESULT  dk_nvenc_encode_filter::GetMediaType(int position, CMediaType *type)
{
...
HRESULT  dk_nvenc_encode_filter::CheckTransform(const CMediaType *itype, const CMediaType *otype)
{
...
HRESULT  dk_nvenc_encode_filter::DecideBufferSize(IMemAllocator *allocator, ALLOCATOR_PROPERTIES *properties)
{
	if (!m_pInput->IsConnected())
		return E_UNEXPECTED;
...
HRESULT dk_nvenc_encode_filter::Transform(IMediaSample *src, IMediaSample *dst)
{
	HRESULT hr = S_OK;
	...
	dk_nvenc_encoder::dk_video_entity_t input;
	input.mem_type = dk_nvenc_encoder::memory_type_host;
	input.data = input_buffer;
	input.data_size = input_data_size;
	input.data_capacity = input_data_size;

	dk_nvenc_encoder::dk_video_entity_t bitstream;
...
STDMETHODIMP dk_nvenc_encode_filter::GetPages(CAUUID *pPages)
{
	...
	pPages->pElems[0] = CLSID_DK_NVENC_ENCODE_FILTER_PROPERTIES;
	return S_OK;
}
