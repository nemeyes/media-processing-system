...
#include "dk_image_source_filter_properties.h"
#include "resource.h"
...
dk_image_source_filter_properties::dk_image_source_filter_properties(LPUNKNOWN unk)
	:CBasePropertyPage(NAME("dk_image_source_filter_properties"), unk, IDD_PROPPAGE_MEDIUM, IDS_PROPPAGE_TITLE)
{

}

dk_image_source_filter_properties::~dk_image_source_filter_properties(VOID)
{

}

CUnknown* WINAPI dk_image_source_filter_properties::CreateInstance(LPUNKNOWN unk, HRESULT *hr)
{
	CUnknown *punk = new dk_image_source_filter_properties(unk);
	if (punk == NULL)
	{
		*hr = E_OUTOFMEMORY;
	}
...
HRESULT dk_image_source_filter_properties::OnConnect(IUnknown *unk)
{
...
HRESULT dk_image_source_filter_properties::OnDisconnect(VOID)
{
	//if (_setting == NULL)
	//	return E_UNEXPECTED;
	//_setting->Release();
...
HRESULT dk_image_source_filter_properties::OnActivate(VOID)
{
	return S_OK;
}

HRESULT dk_image_source_filter_properties::OnDeactivate(VOID)
{
	return S_OK;
}

HRESULT dk_image_source_filter_properties::OnApplyChanges(VOID)
{
	//if (_setting)
	//{
	//	return S_OK;
...
INT_PTR dk_image_source_filter_properties::OnReceiveMessage(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
	HWND hwnd_edit;
	switch (msg)
	{
