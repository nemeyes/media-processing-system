...
#include "dk_msdk_decode_filter_properties.h"
#include "dk_msdk_decode_filter.h"

dk_msdk_decode_filter::dk_msdk_decode_filter(LPUNKNOWN unk, HRESULT *hr)
	: CTransformFilter(g_szFilterName, unk, CLSID_DK_MEDIA_SDK_DECODE_FILTER)
{
	_decoder = new dk_media_sdk_decoder();
}

dk_msdk_decode_filter::~dk_msdk_decode_filter(VOID)
{
	if (_decoder)
	{
		delete _decoder;
...
CUnknown * WINAPI dk_msdk_decode_filter::CreateInstance(LPUNKNOWN unk, HRESULT *hr)
{
	*hr = S_OK;
	CUnknown* punk = new dk_msdk_decode_filter(unk, hr);
	if (punk == NULL)
		*hr = E_OUTOFMEMORY;
	return punk;
}

STDMETHODIMP dk_msdk_decode_filter::NonDelegatingQueryInterface(REFIID riid, void** ppv)
{
...
HRESULT  dk_msdk_decode_filter::StartStreaming()
{
	return NOERROR;
}

HRESULT  dk_msdk_decode_filter::StopStreaming()
{
	return NOERROR;
}

// override this to grab extra interfaces on connection
HRESULT  dk_msdk_decode_filter::CheckConnect(PIN_DIRECTION direction, IPin *pin)
{
...
HRESULT  dk_msdk_decode_filter::BreakConnect(PIN_DIRECTION direction)
{
...
HRESULT  dk_msdk_decode_filter::CompleteConnect(PIN_DIRECTION direction, IPin *pin)
{
	if (direction == PINDIR_INPUT)
	{
		if (_decoder)
...
HRESULT  dk_msdk_decode_filter::AlterQuality(Quality quality)
{
	UNREFERENCED_PARAMETER(quality);
	return S_FALSE;
}

HRESULT  dk_msdk_decode_filter::SetMediaType(PIN_DIRECTION direction, const CMediaType *type)
{
	UNREFERENCED_PARAMETER(direction);
...
HRESULT  dk_msdk_decode_filter::EndOfStream(void)
{
	HRESULT hr = NOERROR;
	if (m_pOutput != NULL)
	{
...
HRESULT  dk_msdk_decode_filter::BeginFlush(void)
{
	HRESULT hr = NOERROR;
	if (m_pOutput != NULL)
	{
...
// or a worker thread
HRESULT  dk_msdk_decode_filter::EndFlush(void)
{
...
HRESULT  dk_msdk_decode_filter::NewSegment(REFERENCE_TIME tStart, REFERENCE_TIME tStop, double dRate)
{
	if (m_pOutput != NULL)
	{
...
HRESULT  dk_msdk_decode_filter::CheckInputType(const CMediaType *type)
{
...
HRESULT  dk_msdk_decode_filter::GetMediaType(int position, CMediaType *type)
{
...
HRESULT  dk_msdk_decode_filter::CheckTransform(const CMediaType *itype, const CMediaType *otype)
{
...
HRESULT  dk_msdk_decode_filter::DecideBufferSize(IMemAllocator *allocator, ALLOCATOR_PROPERTIES *properties)
{
	if (!m_pInput->IsConnected())
		return E_UNEXPECTED;
...
HRESULT dk_msdk_decode_filter::Transform(IMediaSample *src, IMediaSample *dst)
{
	HRESULT hr = S_OK;
...
/*
STDMETHODIMP dk_msdk_decode_filter::SetMediaType(VMXNET_SUB_MEDIA_TYPE media_type)
{
_omedia_type = media_type;
return S_OK;
}

STDMETHODIMP dk_msdk_decode_filter::SetResolution(USHORT width, USHORT height)
{
_owidth = width;
_oheight = height;
return S_OK;
...
STDMETHODIMP dk_msdk_decode_filter::GetMediaType(VMXNET_SUB_MEDIA_TYPE &media_type)
{
media_type = _omedia_type;
return S_OK;
}

STDMETHODIMP dk_msdk_decode_filter::GetResolution(USHORT &width, USHORT &height)
{
width = _owidth;
height = _oheight;
return S_OK;
...
*/
STDMETHODIMP dk_msdk_decode_filter::GetPages(CAUUID *pPages)
{
	if (pPages == NULL)
		return E_POINTER;
	pPages->cElems = 1;
	...
	pPages->pElems[0] = CLSID_DK_MEDIA_SDK_DECODE_FILTER_PROPERTIES;
	return S_OK;
}