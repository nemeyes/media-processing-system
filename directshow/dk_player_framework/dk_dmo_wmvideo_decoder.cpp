...
dk_dmo_wmvideo_decoder::dk_dmo_wmvideo_decoder(void)
{
...
CComPtr<IBaseFilter> dk_dmo_wmvideo_decoder::get_filter(void)
{
	return _decoder;
}

CComPtr<IPin> dk_dmo_wmvideo_decoder::get_output_pin(void)
{
	CComPtr<IPin> outpin;
...
CComPtr<IPin> dk_dmo_wmvideo_decoder::get_input_pin(void)
{
	CComPtr<IPin> inpin;
...
HRESULT dk_dmo_wmvideo_decoder::add_to_graph(CComPtr<IGraphBuilder> graph)
{
	...
	if (_found)
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				if (SUCCEEDED(hr))
				{
					hr = graph->AddFilter(decoder, L"WMVideo Decoder DMO");
...
void dk_dmo_wmvideo_decoder::enum_decoder_dmo(void)
{
	_found = FALSE;
	IEnumDMO* pEnum = NULL;
	...
	if (SUCCEEDED(hr))
	{
		...
		do
		{
			...
			if (hr == S_OK)
			{
				...
				if (wcscmp(wszName, L"WMVideo Decoder DMO") == 0)
				{
					_clsidDMO = clsidDMO;
					_found = TRUE;
				}
